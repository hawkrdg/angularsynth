...
@Component({
  ...
  styleUrls: ['./synth.component.styl']
})
export class SynthComponent implements OnInit {
	@Input() ctx: AudioContext;			// AudioContext that this synth will run in...
	@Input() dest: any;							// Destination to connect to...
	@ViewChild('powerSwitch') powerSwitch: ElementRef;
  ...
  ngOnInit(): void {
		...
		this.synthLevel.gain.value = .4;

		this.synthPanner = this.ctx.createStereoPanner();
		this.synthPanner.pan.value = 0;
